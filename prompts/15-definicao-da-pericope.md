# 15. Definição da Perícope

**Localização no trabalho:** Seção 4.2 — Definição da Perícope

## O que este prompt gera
A justificativa literária e estrutural para os limites exatos da perícope escolhida (por que ela começa e termina onde você delimitou), com base em marcadores textuais (mudanças de interlocutor, de assunto, conectivos, inclusões literárias).

## Fontes recomendadas para esta seção
Comentários bíblicos com análise estrutural detalhada, o próprio texto bíblico ao redor da perícope.
...
1. Uma explicação de como o texto se movimenta nos versículos imediatamente anteriores ao início da perícope proposta, identificando o ponto exato (versículo) em que ocorre uma mudança de assunto, de interlocutor, de tom ou de estrutura que justifica o início da perícope ali.
2. Uma explicação equivalente para o final da perícope: por que ela termina no versículo escolhido, e não antes ou depois, com base em marcadores textuais (conclusão de um argumento, mudança de assunto no versículo seguinte, inclusão literária, etc.).
